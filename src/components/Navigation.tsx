...
const CREATOR_NAV: NavItem[] = [
  { to: '/creator', label: 'Creator Dashboard', icon: LayoutDashboard, roles: ['creator', 'admin'] },
  { to: '/creator/quizzes', label: 'My Quizzes', icon: FileText, roles: ['creator', 'admin'] },
  { to: '/creator/payouts', label: 'Payouts', icon: CreditCard, roles: ['creator', 'admin'] },
  { to: '/creator/reports', label: 'Reports', icon: BarChart3, roles: ['creator', 'admin'] },
];

const ADMIN_NAV: NavItem[] = [
  { to: '/admin', label: 'Admin Dashboard', icon: ShieldCheck, roles: ['admin'] },
  { to: '/admin/applications', label: 'Applications', icon: ListChecks, roles: ['admin'] },
  { to: '/admin/payouts', label: 'Payouts', icon: CreditCard, roles: ['admin'] },
  ...
  { to: '/admin/quizzes', label: 'Quizzes', icon: ScrollText, roles: ['admin'] },
];
...
function filterByRole(items: NavItem[], role: UserRole, isApprovedCreator: boolean) {
  return items.filter((item) => {
    ...
    if (role === 'user' && isApprovedCreator && item.roles.includes('creator')) return true;
    return false;
  });
}
...
const ROLE_LABELS: Record<UserRole, { label: string; variant: 'primary' | 'secondary' | 'warning' }> = {
...
export function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex lg:flex-col w-[260px] shrink-0 h-screen sticky top-0 bg-surface/80 backdrop-blur-xl border-r border-border/50">
          ...
          {userNav.map((item) => {
            ...
            return (
              <NavLink key={item.to} to={item.to} end={item.to === '/'} className={navLinkClass}>
                <Icon className="w-[18px] h-[18px] shrink-0" strokeWidth={2.1} />
      ...
      <div className="p-4 border-t border-border/50">
        <NavLink to="/wallet" className="mb-3 flex items-center gap-2 px-3 py-2.5 rounded-2xl bg-cream/80 hover:bg-cream transition-colors">
        ...
        <NavLink to="/settings" className="flex items-center gap-3 p-2.5 rounded-2xl hover:bg-cream transition-colors">
  ...
  return (
    <header
      className={`lg:hidden sticky top-0 z-40 w-full backdrop-blur-xl transition-all duration-200 safe-top ${
        scrolled ? 'bg-background/85 border-b border-border/50 shadow-soft' : 'bg-background/60'
      }`}
    >
      <div className="flex items-center justify-between h-14 px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onOpenMenu}
            className="h-11 w-11 flex items-center justify-center rounded-2xl bg-surface/70 text-text active:scale-95 transition-transform lg:hidden"
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <Icon
                    className={`w-[22px] h-[22px] ${isActive ? 'scale-105' : ''} transition-transform`}
        ...
        >
          {({ isActive }) => (
            <>
              ...
              <Menu className={`w-[22px] h-[22px] ${isActive ? 'scale-105' : ''} transition-transform`} strokeWidth={isActive ? 2.4 : 2} />
              <span className="text-[11px] font-heading font-medium tracking-tight">Menu</span>
            </>
          )}
        </NavLink>
      </div>
...
export function MenuSheet({ open, onClose }: MenuSheetProps) {
  ...
  const role = currentUser.role;
  const isApproved = currentUser.is_approved_creator;
  const userNavExtra = USER_NAV.filter((i) => !MOBILE_TABS.find((t) => t.to === i.to));
  const creatorNav = filterByRole(CREATOR_NAV, role, isApproved);
  const adminNav = filterByRole(ADMIN_NAV, role, isApproved);

  const itemClass =
    'flex items-center gap-3 h-12 px-4 rounded-2xl text-sm font-heading font-medium text-text-soft hover:bg-surface/70 hover:text-text transition-colors';
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`absolute bottom-0 left-0 right-0 rounded-t-3xl bg-cream shadow-elevated max-h-[85vh] flex flex-col transition-transform duration-300 ease-out safe-bottom ${
      ...
      >
        ...
        <div className="px-5 py-4 flex items-center gap-3 border-b border-border/50">
          <Avatar name={currentUser.full_name} size="lg" ring />
          <div className="flex-1 min-w-0">
            <p className="font-heading font-semibold text-text truncate">{currentUser.full_name}</p>
            <div className="flex items-center gap-2 mt-1">
              <Badge size="sm" variant={ROLE_LABELS[currentUser.role].variant}>
                {ROLE_LABELS[currentUser.role].label}
              </Badge>
              <span className="text-xs text-muted">
                Wallet: <span className="font-semibold text-text">{formatNaira(walletBalance)}</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-6">
          {userNavExtra.length > 0 && (
            <nav className="space-y-1">
              {userNavExtra.map((item) => {
                const Icon = item.icon;
                return (
                  <NavLink
                    key={item.to}
                    ...
                    onClick={onClose}
                    className={({ isActive }) =>
                      `${itemClass} ${isActive ? '!bg-primary !text-cream' : ''}`
                    }
                  >
          ...
          <nav className="space-y-1 pt-2 border-t border-border/50">
            <button className={`${itemClass} w-full text-left !text-danger hover:!bg-danger-bg`}>
              <LogOut className="w-[18px] h-[18px]" strokeWidth={2.1} />
              <span className="flex-1">Log out</span>
            </button>
            <div className={`${itemClass} !text-muted !hover:bg-transparent`}>
